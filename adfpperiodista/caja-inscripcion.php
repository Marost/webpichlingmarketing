...
<dl>
    ...
    <dd>
        <form enctype='multipart/form-data' action="" method="post" id="subscribe-form" style="display:block">
            ...
            <span>
                <input type="hidden" name="evento" id="evento" value="Almuerzo Periodista">
            </span>
            <span>
            	<br />
...
{ 
    ...
    { 
        ...
        { 
            if($result->email == $_POST['email'] and $result->evento == "Almuerzo Periodista")
            { 
                $verificar_usuario = 1;
				}
			else
			{
				if($result->dni == $_POST['DNI'] and $result->evento == "Almuerzo Periodista")
				{
					$verificar_usuario = 1; 
					}
			}
        ...
        { 
			...
			//} 
				...
				echo "<script type=\"text/javascript\">alert('Confirmación exitosa enviada a su correo electrónico. ¡Feliz Dia del Periodista Deportivo!'); window.location='salir.php';</script>";
            }
            else 
            { 
