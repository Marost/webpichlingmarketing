...
<dl>
    ...
    <dd>
        <form enctype='multipart/form-data' action="" method="post" id="subscribe-form" style="display:block">
            ...
            <span>
                <fieldset class="sin_borde">
                    <input type="radio" name="asistencia" value="Solo" /> Con gusto asistiré</br></br>
            ...
            <span>
                <input type="hidden" name="evento" id="evento" value="Almuerzo Bicentenario">
            </span>
            <span>
            	<br />
...
{ 
    ...
    { 
        ...
        { 
            if($result->email == $_POST['email'] and $result->evento == "Almuerzo Bicentenario")
            { 
                $verificar_usuario = 1;
				}
			else
			{
				if($result->dni == $_POST['DNI'] and $result->evento == "Almuerzo Bicentenario")
				{
					$verificar_usuario = 1; 
					}
			}
        ...
        { 
			...
			//} 
				...
				echo "<script type=\"text/javascript\">alert('Confirmación exitosa enviada a su correo electrónico. Próximamente le enviaremos el número de mesa asignado. ¡Feliz Bicentenario Patrio!'); window.location='salir.php';</script>";
            }
            else 
            { 
