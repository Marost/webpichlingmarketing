...
<body>
	<section class="main">
		...
		<p>
			<?php
				$sql = "SELECT * FROM pmkt_registro WHERE asistencia='Solo' AND evento='$clave' ";
				$sql2 = "SELECT * FROM pmkt_registro WHERE asistencia='NoIras' AND evento='$clave' ";
        		$result = mysql_query($sql);
        		$result2 = mysql_query($sql2);
        		echo "Participaran: " . mysql_num_rows($result) . " | No participaran: " . mysql_num_rows($result2);
			?>
		</p>
		...
		<table style="text-align:center; color:white; font-size:18px;">
			...
			<?php
			$consultaCategoria=mysql_query("select * from pmkt_registro where id <> 0 and evento='$clave' order by `id` asc");
