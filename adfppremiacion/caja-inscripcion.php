...
{ 
    ...
    { 
        ...
        { 
            if($result->email == $_POST['email'] and $result->evento == $clave)
            { 
                $verificar_usuario = 1;
				}
			else
			{
				if($result->dni == $_POST['DNI'] and $result->evento == $clave)
				{
					$verificar_usuario = 1; 
					}
			}
        ...
        { 
			...
			//} 
				...
				$sql = "INSERT INTO pmkt_registro (nombre_apellido, email, DNI, asistencia, pareja, evento) VALUES ('$nombre', '$email', '$DNI', '$asistencia', '$pareja', '$actividad')";
                mysql_query($sql); 
				...
				$_SESSION["evento"]=$actividad;

                //echo $_SESSION["nombre_apellido"].'  usted se ha registrado correctamente.';

				echo "<script type=\"text/javascript\">alert('Confirmación exitosa enviada a su correo electrónico. ¡Muchas gracias!'); window.location='salir.php';</script>";
                include ("correo.php");
            }
            else 
            { 
